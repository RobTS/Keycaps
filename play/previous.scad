    ...
    difference()
    {
        ...
        union()
        {
            //translate(obj_pos) rotate(obj_rot) scale(obj_scale) import(obj);               
				rotate([-head_tilt,180,0]) translate([-1,0,0]) linear_extrude(height = w, center = true, convexity = 10, twist = 0					)
				polygon(points=[[-5,-6],[1,1],[-5,8]], paths=[[0,1,2]]);
				rotate([-head_tilt,180,0]) translate([5.5,0,0]) linear_extrude(height = w, center = true, convexity = 10, twist = 0					)
				polygon(points=[[-5,-6],[1,1],[-5,8]], paths=[[0,1,2]]);
				// end design
            difference()
            {
