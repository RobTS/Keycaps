...
// decoration
obj = "semiquaver.stl";          // decoration object
obj_pos = [-3.25,-4.5,5.5];        // decoration offset
factor_em = 0.1;
obj_scale = [1.1*factor_em,1.1*factor_em,1.3*factor_em]; // decoration scale
...
module key()
{
    difference()
    {
        ...
        union()
        {
				rotate([head_tilt,0,0]) translate([-1,0,0]) linear_extrude(height = w, center = true, convexity = 10, twist = 0					)
				polygon(points=[[-5,-6],[1,1],[-5,8]], paths=[[0,1,2]]);
				rotate([head_tilt,0,0]) translate([5.5,0,0]) linear_extrude(height = w, center = true, convexity = 10, twist = 0					)
				polygon(points=[[-5,-6],[1,1],[-5,8]], paths=[[0,1,2]]);
				// end design
// end design
            difference()
